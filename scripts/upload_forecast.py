...
# # Load data
# data = pd.read_csv('../data/large_weather_data.csv')  # Adjust path if needed
...
# # Handle missing values (option: fill with column mean)
# X = X.fillna(X.mean())

# # Predict temperature
# data['predicted_temp'] = model.predict(X)
# data['predicted_humidity'] = model2.predict(X)
# data['predicted_wind_speed'] = model3.predict(X)
# data['predicted_precipitation'] = model4.predict(X)

# # Optional: Round the prediction
# data['predicted_temp'] = data['predicted_temp'].round(0)
# data['predicted_humidity'] = data['predicted_humidity'].round(0)
# data['predicted_wind_speed'] = data['predicted_wind_speed'].round(1)
# data['predicted_precipitation'] = data['predicted_precipitation'].round(1)
